...
class HAN(Model):
    ...
    def build_sent_encoder(self):
        ...
        text_input = Input(shape=(self.max_sent_length,512,), dtype='float32')
        #self.model_word=self.build_word_encoder()
        #sent_encoder=TimeDistributed(self.model_word)(text_input)
        
        sent_encoder=TimeDistributed(Dense(DENSE_SIZE))(text_input)
        
        # For Bidirectional, devide by 2
        encoded_text = Bidirectional(
            GRU(int(self.sent_embed_dim / 2), return_sequences=True,activation='sigmoid', dropout=0.5)
        )(sent_encoder)

        dense_text = TimeDistributed(Dense(DENSE_SIZE))(encoded_text)
        doc_att = Attentions(name='sent_attention')(dense_text)
        return Model(text_input, doc_att)
    
    def build_doc_encoder(self, doc_encoder):
        
        encoded_doc = Bidirectional(
            GRU(int(self.doc_embed_dim / 2), return_sequences=True,activation='sigmoid', dropout=0.5)
        )(doc_encoder)
        dense_doc = TimeDistributed(Dense(DENSE_SIZE))(encoded_doc)
        document_att = Attentions(name='doc_attention')(dense_doc)
        return document_att
    ...
    def train_model(self, checkpoint_path, X_train, y_train, X_val, y_val, X_test, y_test,
                    ...
                    metric='acc', monitor='val_acc', batch_size=20, epochs=10):
        """Train the HAN model.
        ...
        return results, precision, recall, f1, kappa, auc, matrix
